(function() {
	...
	Agreement.$inject = ['$state', '$stateParams', '$mdDialog', '$mdMedia', 'agreementManager', 'agreementSharedData', '$scope', 'parentModel', 'pageStructureFactory'];
	...
	function Agreement($state, $stateParams, $mdDialog, $mdMedia, agreementManager, agreementSharedData, $scope, parentModel, pageStructureFactory) {
		...
		if ($state.current.name === 'home.agreement.viewAll') {
			...
			pageStructureFactory.getLayout('agreement_viewAll')
				.then(
					function(response) {
		...
		if (($state.current.name === 'home.agreement.create') || ($state.current.name === 'home.agreement.QuickCreate')) {
			...
			if ($state.current.name === 'home.agreement.QuickCreate') {
				...
				pageStructureFactory.getLayout('agreement_CRUD')
					.then(
						function(response) {
							vm.agreementFields = response.data;
